...
<body class="bg-gray-50 dark:bg-gray-900 font-sans antialiased transition-colors duration-300">
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden bg-gray-50 dark:bg-gray-900">
            ...
            <header class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl shadow-sm border-b border-gray-100 dark:border-gray-700 h-20 flex items-center justify-between px-8 z-10 sticky top-0">
                <div class="flex items-center md:hidden">
                    ...
                    <button class="text-gray-500 hover:text-gray-700 focus:outline-none" aria-label="Open mobile menu">
                ...
                <div class="flex items-center justify-end w-full space-x-6">
                    ...
                    <!-- Language Toggle -->
                    <a href="{{ route('lang.switch', app()->getLocale() == 'es' ? 'en' : 'es') }}" class="text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm px-3 py-2 font-medium transition-colors" aria-label="Switch language to {{ app()->getLocale() == 'es' ? 'English' : 'Español' }}">
                        {{ strtoupper(app()->getLocale()) }}
                    </a>
                    
                    <!-- Theme Toggle -->
                    <button id="theme-toggle" type="button" class="text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-xl text-sm p-2.5 transition-colors" aria-label="Toggle dark mode">
    ...
    <div id="deleteModal" class="fixed inset-0 z-[9999] hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0" data-modal-overlay="true">
            <!-- Background overlay -->
            <div class="fixed inset-0 bg-gray-500 dark:bg-gray-900 bg-opacity-75 dark:bg-opacity-80 transition-opacity backdrop-blur-sm" aria-hidden="true" data-modal-overlay="true"></div>

            <!-- Center modal -->
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>

            <!-- Modal panel -->
            <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl px-4 pt-5 pb-4 text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6 border border-gray-100 dark:border-gray-700 relative">
                <div class="absolute top-0 right-0 pt-4 pr-4">
                    <button type="button" id="closeModal" aria-label="Close" class="bg-white dark:bg-gray-800 rounded-md text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                ...
                <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse gap-3">
                    <form id="deleteForm" method="POST" class="w-full sm:w-auto">
                        @csrf
                        @method('DELETE')
                        <button type="submit" id="confirmDelete" class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-lg shadow-red-500/30 px-4 py-2.5 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:w-auto sm:text-sm transition-all hover:scale-105">
                            {{ __('Delete') }}
                        </button>
                    </form>
